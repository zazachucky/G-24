...
const out = path.join(root, 'assets/video/home-shopping');
const delay = ms => new Promise(resolve => setTimeout(resolve, ms));

(async () => {
  const videoPath = path.join(out, 'gs-ai-live-home-shopping-120s.mp4');
  if (!fs.existsSync(videoPath)) throw new Error('Home shopping MP4 is not ready: ' + videoPath);
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'gs-home-shopping-browser-'));
  ...
  try {
    ...
    await send('Page.navigate', { url: pathToFileURL(path.join(root, 'preview/home-shopping.html')).href });
    ...
    const results = await evaluate(`(async () => {
      ...
      ok('home shopping timeline loaded',Array.isArray(window.GS_HOME_VIDEO?.scenes));
      ok('home shopping MP4 selected',v.currentSrc.endsWith('/assets/video/home-shopping/gs-ai-live-home-shopping-120s.mp4'));
      ...
      await seek(45.5);
      ok('size chapter updates',document.querySelector('.chapter[aria-current="true"]')?.dataset.sceneId==='S05_SIZE_GUIDE');
    ...
    const report = { status: 'PASS', checked_at: new Date().toISOString(), scope: 'Home shopping MP4 browser playback and reference player; host animation quality and Video AI models are not evaluated by this browser check', ...results, mobile };
