...
2026-09-21. 첨부 Customer·Director 화면의 누락 기능과 개인 사이즈 입력·실측 비교를 추가하고 **Python 112개와 실제 Chrome 311개 PASS**를 확인했다. 영상 기능은 검수한 화면 7개 구간과 샘플 제작 대본 12개 장면의 제한된 조회·요약·탐색으로 확인했다. 외부 영상 이해 모델이나 임의 영상 분석의 성능을 검증한 것이 아니며, 기존 reference 테스트 결과를 UI 결과로 대신하지 않았다.

| 검증 | 결과 | 실제 증거 |
| --- | --- | --- |
| 상품·이미지·리뷰·코디 Fixture 검증 | PASS | [기존 데이터 보고서](verification-report.json) |
| Python 단위·서비스·HTTP 검사 | 112개 PASS | [실행 출력](evidence/experience-unit-output.txt) |
...
Chrome 311개는 기존 6종 194개(59+12+31+25+21+46)와 신규 Customer 50개·Director 21개·영상 24개·사이즈 입력 22개의 합계다. 각 JSON의 `completed_at`이 실제 검사 완료 시각이며, 3회 정상 시연은 59개 통합 검사 안에 포함된다. 이전 Python 55개·61개 및 Chrome 194개 완료 기록과 1차 확장 100개·289개 결과는 당시 범위의 이력으로 보존한다.

기본 8765 서버를 최신 코드로 재시작하고 **2026-09-21 10:25:52 UTC**에 실제 실행을 확인했다. 고객과 Director의 모니터링·고객 이용·액션 기록·효과 분석이 같은 RUN `54adbd46c5c7408296510ff73cd5a599`에 연결됐고, 네 Director 화면의 1440px·390px 검사에서 가로 넘침·HTTP 오류·브라우저 예외가 없었다. 실제 누적 4개 행과 추가 전후 지표 3개가 존재하며 고정 Simulation은 승인·결과 공개 전 숨김 상태였다. 서버 재시작으로 이전 메모리 시연 기록은 초기화됐다. [이번 확장의 실행 확인](evidence/experience-live-runtime.json)과 [코드·검사·실행 감사](evidence/experience-completion-audit.json)를 현재 적용 근거로 사용한다. 이전 09:51 UTC의 [서버 확인](evidence/dashboard-live-runtime.json)과 [구좌 감사](evidence/dashboard-audit.json)는 당시 61개·194개 검사 및 서버 재시작 이력이다.

검증 명령과 재실행 방법은 [실행·시연 안내](run-demo.md)를 참조한다. 독립 검사 서버를 사용하며 같은 서버의 데모 상태를 사용하는 브라우저 검사는 순차 실행한다.

## 최신 요청: 첨부 화면에서 빠진 기능 추가
